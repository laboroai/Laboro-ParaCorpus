...
snakemake --nolock --snakefile "$(dirname "$0")/snakemake/Snakefile" ${KEEPGOING} --configfile "${SNAKEFILE}" -j "${NUMJOBS}" ${CLUSTERCOMMAND} ${CLUSTERCONFIG} --config bitextor="$(dirname "$0")"