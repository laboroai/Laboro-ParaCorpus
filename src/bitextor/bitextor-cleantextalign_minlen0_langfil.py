...
import sys
import argparse
import judge_language_jp as jljp
...
oparser.add_argument("-e", "--en-minlen",
                     help="minimun length of the English text",
                     type=int, dest="en_minlen", default=0)
oparser.add_argument("-j", "--ja-minlen",
                     help="minimun length of the Japanese text",
                     type=int, dest="ja_minlen", default=0)
...
for i in reader:
    ...
    if options.en_minlen>=0:
        if len(campos[2].strip().split())<=options.en_minlen:
            continue
    if options.ja_minlen>=0:
        if len(campos[3].strip())<=options.ja_minlen:
            continue
    is_jp,_ = jljp.judge(campos[3].strip())
    if not is_jp:
        continue
