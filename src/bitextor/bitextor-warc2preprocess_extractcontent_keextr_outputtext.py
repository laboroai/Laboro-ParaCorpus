...
oparser.add_argument("--parser", dest="parser", default="bs4", choices={'bs4', 'modest', 'alcazar', 'simple', 'extractcontent'},
...
for record in f:
    ...
    if options.boilerpipe:
        ...
        extractor = ExtrB(extractor='KeepEverythingExtractor', html=text)
        deboiled = str(extractor.getHTML())
        plaintext = str(extractor.getText())
    else:
        deboiled = text
        parser = SimpleParser()
        try:
    ...
    elif options.parser == "extractcontent":
        if lang == 'ja':
            logging.info(url + ": Getting text with ExtractContent")
            try:
                extractor = extractcontent.ExtractContent()
                extractor.analyse(deboiled)
                plaintext,_ = extractor.as_text()
            except:
                logging.info("Something went wrong when using ExtractContent. Ignoring this document")
                plaintext = ''
        elif lang == 'en':
            logging.info(url + ": Getting text with modest (selectolax)")
            try:
